...
var haikuGrab = {q: "#haikuchallenge lang:en -filter:retweets", count: 10, result_type: "recent"}; 
...
function uwuPost() {
	T.get('search/tweets', haikuGrab, function (error, data) {
	  ...
	  if (!error) {
		...
		T.post('statuses/update', {status: haikUWU, attachment_url:`https://twitter.com/${data.screen_name[0]}/status/${haikuOrig}`}, function (error, response) {
			if (response) {
